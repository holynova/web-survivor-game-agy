...
export class MenuScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    const subTitle = this.add.text(width / 2, 92, '幽冥百鬼围攻，化厨具为神兵！构筑你的战斗厨房！', {
      fontSize: '13px',
      color: '#8fa3a6',
    });
  ...
  private createCard(
    ...
    // 角色名
    const nameText = this.add.text(0, -h / 2 + 20, charDef.nameKey, {
      fontSize: '17px',
      color: '#f4a261',
      fontStyle: 'bold',
    });
    ...
    const startWeaponText = this.add.text(0, -h / 2 + 50, `初始厨具: ${weaponName}`, {
      fontSize: '12px',
      color: '#2a9d8f',
      fontStyle: 'bold',
    });
    ...
    // 描述
    const descText = this.add.text(0, -h / 2 + 80, charDef.descriptionKey, {
      fontSize: '11px',
      color: '#d8e2dc',
      wordWrap: { width: w - 24 },
      align: 'center',
