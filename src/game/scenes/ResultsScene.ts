...
export class ResultsScene extends Phaser.Scene {
  ...
  public create(data: ResultsData): void {
    ...
    // 3. 胜负横幅
    const bannerColor = data.isVictory ? '#ffd166' : '#e63946';
    const titleText = data.isVictory ? '🎉 恭喜！夜市大获全胜！' : '💀 营业结束！摊位被百鬼淹没';
  ...
  private renderBasicStatsCard(
    ...
    const mins = Math.floor(data.stats.timeSurvivedSec / 60);
    const secs = Math.floor(data.stats.timeSurvivedSec % 60);
    const timeStr = `${mins}分${secs.toString().padStart(2, '0')}秒`;

    const infoList = [
      `到达波次: 第 ${data.waveReached} 波 / 共 12 波`,
      `存活时间: ${timeStr}`,
      `驱除妖怪: ${data.stats.totalKills} 只`,
      `造成总伤害: ${data.stats.totalDamageDealt}`,
      `赚取食材: 🥟 ${data.stats.ingredientsEarned}`,
    ];
  ...
  private renderDamageStatsCard(
    ...
    // 武器伤害占比列表
    const totalDmg = Math.max(1, data.stats.totalDamageDealt);
    const weaponEntries = Object.entries(data.stats.damageByWeapon).sort((a, b) => b[1] - a[1]);
    ...
    // 激活的菜谱
    const recipeNames = data.activeRecipes.map(r => `【${r.transformation.transformedNameKey}】`).join(' ');
